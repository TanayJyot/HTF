...
def reservoir_sampling(stream, n):
    """
    Proper reservoir sampling that processes exactly n items from the stream.
    This ensures we only look at exactly n items total.
    """
    reservoir = []

    # Process exactly n items and no more
    for i, review in enumerate(stream.take(n)):
        reservoir.append(review)

    # Shuffle the reservoir for randomness
...
def load_datasets():
    ...
    # Load meta dataset (just a small subset to match 10,000 reviews)
...
def main():
    ...
    # Reservoir sampling to take exactly 10,000 shuffled reviews
    print("Shuffling reviews using reservoir sampling...")
    review_subset = reservoir_sampling(review_dataset, 100000000)
    ...
    # Process items - FIXED: Use take() instead of slicing for streaming dataset
